...
3. 成功：
   ...
   - 服务端确认成功后优先释放 `_report_in_flight`，让下一轮计时继续推进，再做本地背包、日志和进度展示更新。
   - 增加 `herb_gathering` 术法使用次数时，只更新当前打开的术法详情使用次数，不重建整页术法缩略卡。
4. 失败：
...
- 百草山采集属于生产循环，report 成功后不能被非必要 UI 重建拖住下一轮计时。
- `herb/report` 返回的 `drops_gained` 是本地即时更新背包和日志的依据；本流程不走 `refresh_all_player_data` 全量刷新。
- 入背包时使用静默物品获得日志，避免和生产日志重复出现“获得物品”文案。
- `herb_gathering` 术法使用次数只影响术法详情弹窗中的升级条件；缩略卡不展示该字段，因此不重建整页术法 UI。
- 如果术法详情弹窗正打开且查看的是 `herb_gathering`，`SpellSystem.spell_used` 只刷新这个可见弹窗；没有打开详情时，不做任何术法 UI 重建。
