...
- 术法列表改为一行 `4` 个卡片，自动换行，避免手机纵向界面里 5 列过挤。
- 列表区域只允许纵向滚动，不允许横向滚动。
- 卡片固定高度，右上角星级使用 `★N` 徽章；无星不显示徽章，也不占大块星级行。
- 卡片展示内容：
  - 顶部稀有度色条
  - 已装备徽章（仅已装备术法显示）
  - 星级徽章（仅有星时显示）
  ...
  - 五行属性图标/文字（`图标 + 金木水火土无`）
  - 当前等级或未获取状态
  - `装备/卸下` 按钮（生产术法不显示底部按钮）
- 稀有度通过名称颜色、顶部色条和卡片边框体现，直接复用物品稀有度颜色规则。
- 已装备卡片使用更明显的金色边框，但边框宽度保持不变，避免装备/卸下后布局抖动。
- 详情入口统一为点击整张卡片；不再显示 `查看` 按钮。
- 排序规则：
  - 稀有度：`凡 -> 黄 -> 玄 -> 地 -> 天`
  - 同稀有度内：`无 -> 金 -> 木 -> 水 -> 火 -> 土`
- 整张卡片可点击打开详情弹窗，五行底托、图标和文字不能拦截整卡点击。
- 点击详情与拖拽判定分离，拖拽距离超过阈值后不触发详情。
- 缩略卡装备按钮不使用点击缩放动效，只在装备/卸下成功后对目标卡片播放一次轻闪反馈。
...
- 生产循环带来的使用次数变化只更新详情弹窗中的使用次数行；缩略卡不展示使用次数，不应重建整页术法卡。
...
5. 单次进入术法页只应触发一次 `GET /game/spell/list` 和一次 `spell_ui rebuild`。

实现约束：

- 术法子 Tab 点击只由 `NeishiModule.on_spell_tab_pressed -> show_spell_panel` 接管。
- `SpellModule` 不再直接连接 `spell_tab.pressed`，避免一个点击被多个模块重复响应。
- `NeishiModule.show_spell_panel()` 只通过 `_show_sub_panel(spell_panel)` 间接触发一次 `SpellModule.show_tab()`，不要额外再手动调用一次。
- `_refresh_spell_from_server()` 只负责请求与写入 `spell_system / spell_data`，不直接重建 UI；由外层 `show_tab()` 或术法操作完成函数统一调用 `update_spell_ui()`。

### 2) 点击术法卡片

1. 记录 `current_viewing_spell`。
2. 整卡点击可直接打开详情，包括五行底托、图标和文字区域。
...
2. 成功后刷新术法列表与详情。
3. `卸下` 按钮使用红色视觉强调。
4. 缩略卡按钮不做按压缩放，避免和卡片池重建、边框状态变化叠加造成“卡片放大”的错觉。
5. 成功后只对目标缩略卡播放轻闪反馈，不重播整页入场动画。
...
1. 炼丹每轮成功上报会增加 `alchemy` 使用次数。
2. 百草山每轮成功上报会增加 `herb_gathering` 使用次数。
3. `SpellSystem.spell_used` 通知 `SpellModule.on_spell_used` 后，只在 `current_viewing_spell` 等于该术法时刷新详情弹窗的使用次数文本。
4. 不调用 `update_spell_ui()` 重建全部缩略卡，避免生产循环被无关 UI 重建拖慢。

### 6) 被全量同步更新底层数据

1. `refresh_all_player_data` 会先把服务端返回的 `spell_system` 写入本地模型。
2. 如果本次刷新没有要求立即重建术法列表，但术法详情弹窗正打开，则调用 `refresh_visible_detail_popup()` 单独刷新弹窗。
3. 这类刷新不触发 `spell_ui rebuild`；隐藏的术法列表等用户切回术法页时再按最新模型渲染。
...
- 使用次数只出现在详情弹窗升级条件中；如果未来要把使用次数展示到缩略卡，再同步调整生产循环的刷新策略。
- 开局术法的双属性效果必须遍历 `effects[]`，不能只取第一条。
- 当前可见的术法详情弹窗属于即时 UI，必须跟随全量同步后的 `spell_system` 刷新；术法缩略卡列表属于页面级 UI，非当前页面时不要顺手重建。
- 如果调试日志中一次进入术法页出现多条 `GET /game/spell/list` 或一次请求后出现多条 `spell_ui rebuild`，优先检查上述入口所有权和重建所有权是否被破坏。
