# 客户端 GUT API 化测试

## 测试目标

- 用真实服务端 API 验证客户端模块行为。
- 默认不以本地 `scripts/core/*` 旧逻辑作为业务真值。

## 前置条件

- 本地服务端已启动并可访问：`http://localhost:8444/api`
- 测试账号可用：`test / test123`
- 服务端支持 `/api/test/*` 测试接口

## 统一入口

在客户端根目录运行：

```bash
./run_tests.sh
```

可选环境变量：

```bash
export GODOT_BIN=/Applications/Godot.app/Contents/MacOS/godot
export GODOT_TEST_HOME="$(pwd)/.godot_test_home"
```

## 测试流程约定

每条 API 集成测试默认执行：

1. 清理本地 token / server_config
2. 登录测试账号
3. 调用 `/api/test/reset_account`
4. 按需调用 `/api/test/apply_preset`
5. 同步并断言模块状态、日志、文案

## 当前覆盖重点

- 修炼与突破
  - 连续非法上报仅提示一次
  - 非法后等待下一次 5 秒窗口再合并上报（不立即重试）
- 储纳与道具使用
  - 重要物品丢弃确认弹窗（确认/取消）
  - 批量使用走单请求 `inventory/use + count`（非客户端循环 N 次请求）
  - 批量返回支持部分成功语义（`INVENTORY_USE_PARTIAL_SUCCEEDED`）
- 术法操作与战斗锁定
- 炼丹开炉/上报/停火
  - report 失败后的预扣回滚
- 历练模拟/结算/状态收敛
  - 连战等待态推进
- 百草山采集
  - 采集点卡片渲染、互斥提示、开始/report/停止主链路
- 仙务司任务
  - 每日/新手双页签渲染
  - 领取后全量刷新
  - 已领取任务下沉排序
  - 任务二级数字红点即时亮灭
- 邮箱红点
  - `mail_list.unread_count` 摘要信号
  - `邮箱按钮` 数字角标
  - `邮箱按钮 -> 设置 Tab` 红点透传
- 设置（改昵称、排行榜、登出）
- 顶部账号弹窗
  - 改昵称 API 化已覆盖（成功/非法昵称/鉴权技术错误链）
  - 改头像当前为手工回归项（选中高亮、提交成功、顶部头像刷新）
  - 设置页本地配置项（帧率按钮组、音频滑条/静音、占位按钮）当前为手工回归项
- 跨模块 smoke
- 网络层技术错误链
  - `AUTH_TOKEN_INVALID` / `AUTH_KICKED_OUT` 统一处理
  - 技术错误 UI 提示节流
  - `alchemy_report` / `lianli_finish` 单次延迟重试策略
- 通用点击防抖
  - `ClickDebounceUtils`（0.2s）单测覆盖：首次放行、窗口内拦截、窗口后放行、按 key 隔离、重置行为

## 当前未自动化覆盖的 UI 风险

以下风险目前仍主要依赖人工验收：

- 长屏 / 异形屏下的主界面构图稳定性
- 顶部刘海 / 灵动岛 / Home Indicator 安全区遮挡
- 弹窗在安全区内的最终居中表现
- 字体、图标、按钮模板在不同页面的最终视觉一致性

这些内容已补充到：

- [长屏 / 异形屏人工验收与预览工具说明](./UI_Resolution_Manual_Checklist.md)
- [UI 自动化测试规划建议](./UI_Automation_Roadmap.md)

## 当前人工 UI 验收建议

每次影响布局、字体、按钮模板、安全区、弹窗尺寸时，建议至少人工抽查：

1. `1080×1920`
2. `1080×2400`
3. `1125×2436`

如果改动涉及红点入口位置，还应额外检查：

1. `地区 Tab` 右上角小红点不遮挡“地区”文字
2. `设置 Tab` 右上角小红点不遮挡“设置”文字
3. `仙务司` 与 `邮箱` 的数字红点不遮挡按钮主文案
4. 二级入口数字角标不影响点击热区

调试入口：

- `res://scenes/debug/ResolutionPreview.tscn`

## 当前人工性能/手感验收建议

手机端体感卡顿不一定能被 GUT 覆盖。遇到“服务端很快返回，但 UI 过一会儿才动”的问题时，建议用 debug APK 的 `调试` 日志频道排查：

1. 打开 `调试` 筛选，只保留性能日志。
2. 点击目标入口或按钮，观察一次操作对应的网络请求数量。
3. 对照请求后的 UI rebuild 日志，确认是否出现无关页面重建。
4. 若出现多次相同 API，优先排查信号重复连接或页面 show 函数重复调用。
5. 若一次 API 后出现多次相同 rebuild，优先排查数据刷新函数和外层流程是否都在重建 UI。
6. 使用 `复制调试` 导出最后 100 条日志，便于复盘完整时序。

当前已用该方法定位过：

- 进入储纳页时误触发术法页等非当前页面延后刷新。
- 进入术法页时重复连接/重复 show 导致一次点击触发多次 `GET /game/spell/list`。
- 术法数据请求返回后由内外两层各自 `update_spell_ui()`，导致一次请求后重复重建术法列表。

## 后续 UI 自动化方向

建议后续补一层“结构型 UI smoke”，优先检查：

- 关键节点存在与可见
- 关键节点是否落在安全区 / 可视区内
- 弹窗是否居中且未越界
- 多分辨率下主界面、设置、术法弹窗、账号弹窗是否稳定
