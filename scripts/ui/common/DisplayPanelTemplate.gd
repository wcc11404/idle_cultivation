...
static func apply_to_row(header_row: HBoxContainer, config: Dictionary = {}) -> void:
	...
	var current_accent_color := Color(0.870588, 0.705882, 0.207843, 1.0)
	var current_accent_size := Vector2(4.0, 24.0)
	...
	var current_title_color := Color(0.22, 0.2, 0.18, 1.0)
	var current_title_size := 22
	if title is Label:
	...
	var current_line_color := Color(0.82, 0.78, 0.71, 1.0)
	if line is HSeparator:
		current_line_color = line.self_modulate

	var current_row_separation := header_row.get_theme_constant("separation")
